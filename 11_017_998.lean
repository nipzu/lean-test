...
abbrev TM_11017998 := TuringMachine transition

-- In state A and tape filled with zeroes 
def starting_machine : TM_11017998 := 
...
def M (nl nml nmr nr : Nat) : TM_11017998 :=
  let left_tape : Stream' Symbol2 :=
    nml ** [Symbol2.V1] ++
            Symbol2.V0  ::
    nl  ** [Symbol2.V1] ++ₛ
...
lemma reaches_B0 : starting_machine =>> B 0 := by
  exists 1
  ext : 1
  . ext : 1
    . have t2 : (B 0).tape.left_half = [Symbol2.V1] ++ₛ ([Symbol2.V0] ++ₛ Stream'.const Symbol2.V0) :=
        by apply append_assoc
      simp [t2, cons_to_const]
      rfl
    repeat rfl
  . rfl
...
lemma B'_to_M_ (h : nml + nmr ≠ 0) : B' 0 (nml + nmr - 1) =>> M 0 nml nmr (nml + nmr - 1) := by
  cases nmr with
  | zero =>
    exists 2
    simp at h
    have ⟨nml', hnml'⟩ := Nat.exists_eq_succ_of_ne_zero h
    rw [hnml']
    simp
    ext : 1
    . ext : 1
      . unfold M; simp
        have t1 : (TuringMachine.advance^[2] (B' 0 nml')).tape.left_half = 
           Symbol2.V1 :: (0 + nml') ** [Symbol2.V1] ++ₛ Stream'.const Symbol2.V0 := rfl
        simp [Nat.zero_add] at t1
        have t2 : Symbol2.V0 :: 0 ** [Symbol2.V1] ++ₛ Stream'.const Symbol2.V0 = Stream'.const Symbol2.V0 := by
          apply cons_to_const
  ...
  | succ nmr' =>
    ...
    have t2 : (nml + 1) + nmr' ≠ 0 := by simp
    calc B' 0 (nml + (nmr' + 1) - 1)
      _ =   B' 0 ((nml + 1) + nmr' - 1) := by rw [t1]
      _ =>> M 0 (nml + 1) nmr' ((nml + 1) + nmr' - 1) := B'_to_M_ t2
      _ =   M 0 (nml + 1) nmr' (nml + nmr') := by simp
      _ =>> M 0 nml (nmr' + 1) (nml + (nmr' + 1) - 1) := by exists 1
...
lemma B_to_M' : B (nl + nr) =>> M' (nl + 1) (nr + 2 * nl) 0 nr := by
  cases nl with
  | zero =>
    have t1 : 0 + (nr + 1) ≠ 0 := by simp
    simp
    calc B nr
      _ = B (0 + nr) := by simp
      _ =>> B' 0 nr := B_to_B'_
      _ =   B' 0 (0 + (nr + 1) - 1) := by simp
      _ =>> M 0 0 (nr + 1) (0 + (nr + 1) - 1) := B'_to_M_ (nml := 0) (nmr := nr + 1) t1
      _ =   M 0 0 (nr + 1) nr := by simp
      M 0 0 (nr + 0 + 1) nr =>> M' 1 nr 0 nr := M_to_M'_ (nl := 0) (nml := nr) (nmr := 0) (nr := nr)
  | succ nl' =>
    have t1 : (nl' + 1) + nr = nl' + (nr + 1) := by linarith
    have t2 : nr + 1 + 2 * nl' + 2 = nr + 2 * (nl' + 1) + 0 + 1 := by linarith
    calc B ((nl' + 1) + nr)
      _ = B (nl' + (nr + 1)) := by rw [t1]
      _ =>> M' (nl' + 1) (nr + 1 + 2 * nl') 0 (nr + 1) := B_to_M' (nl := nl') (nr := nr + 1)
...
theorem B_to_B : B n =>> B (n + 1) := by
  calc B n
    _ =  B (n + 0) := by simp
    _ =>> M' (n + 1) (0 + 2 * n) 0 0 := B_to_M'
    _ =>> B (n + 1) := by simp; exists 3
...
theorem tm_not_halt : TuringMachine.does_not_halt starting_machine :=
  tm_not_halt' B starting_machine @B_to_B @B_ne_B reaches_B0
